...
def navigation_callback(msg):
    ...
    if msg.navigation_status:
        ...
        finder = AStarFinder(heuristic=octile, diagonal_movement = 1,weight = 2)
        path, runs = finder.find_path(start,end,grid)
        path = simpify_path(path)
        print(path)
        vertice = []
        for item in path:
...
def main():
    ...
    pubVert = rospy.Publisher("/vertices", vertices, queue_size = 1, tcp_nodelay = True)
    rospy.spin()
